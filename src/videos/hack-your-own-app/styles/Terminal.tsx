...
// Terminal / security-log aesthetic — fits a video about hacking your own app.
// Monospace, scanlines, live typing, subtle keystroke SFX, script-driven step
// icons, a progress bar, and an outro CTA.
...
// the text isn't sitting on flat black. CSS gradient only: the cloud render
// container has no GPU, so keep this composition WebGL2-free.
...
// The steps section builds a "hack your own app" function in a code editor
// panel that types itself out line-by-line, one call appended per real step.
// We invent the function names (no bodies) — it's pseudo-code that says exactly
...
const SIG: Tok[] = [
  { t: "def ", c: SYN.kw },
  { t: "hack_my_own_app", c: SYN.fn },
  { t: "():", c: SYN.pun },
];
const CALL_ZAP: Tok[] = [
  { t: "    ", c: SYN.pun },
  { t: "run_zap_scan", c: SYN.call },
  { t: "(", c: SYN.pun },
  { t: "target", c: SYN.arg },
  { t: "=", c: SYN.pun },
  { t: '"staging"', c: SYN.str },
...
const CALL_CI: Tok[] = [
  { t: "    ", c: SYN.pun },
  { t: "scan_every_commit", c: SYN.call },
  { t: "(", c: SYN.pun },
  { t: "ci", c: SYN.arg },
  { t: "=", c: SYN.pun },
  { t: "True", c: SYN.kw },
    ...
    <AbsoluteFill style={{ alignItems: "center", paddingTop: 205 }}>
      ...
      >
        ...
        >
          ...
          <span style={{ color: SYN.cmt, fontSize: 22, marginLeft: 12 }}>hack_my_own_app.py</span>
        </div>
...
const HOOK_BEAT: ImageBeatMeta = {
  id: "hook",
  file: "hack-your-own-app/images/hook-hack.jpg",
  label: "TARGET: YOUR OWN APP — STAGING",
...
const STEP_BEATS: ImageBeatMeta[] = [
  {
    id: "step1",
    file: "hack-your-own-app/images/step1-zap.jpg",
    label: "OWASP ZAP — ACTIVE SCAN REPORT",
    width: 1000,
    bottom: 120,
  },
  {
    id: "step2",
    file: "hack-your-own-app/images/step2-burp.jpg",
    label: "BURP SUITE — INTERCEPTED REQUEST",
    width: 1000,
    bottom: 120,
  },
  {
    id: "step3",
    file: "hack-your-own-app/images/step3-ci.jpg",
    label: "CI — SECURITY SCAN ON EVERY PUSH",
    width: 1000,
    bottom: 120,
  },
];
...
export const Terminal: React.FC<AudioSyncedProps> = ({ narrationEndFrame }) => {
  ...
  // Build the accumulating "hack your own app" function, one call per step.
  const codeLines: CodeLine[] =
    stepRanges.length === 3
      ? [
          { atFrame: stepRanges[0].start, toks: SIG },
          { atFrame: stepRanges[0].start + 20, toks: CALL_ZAP },
          { atFrame: stepRanges[1].start, toks: CALL_BURP },
          { atFrame: stepRanges[2].start, toks: CALL_CI },
          { atFrame: stepRanges[2].start + 45, toks: RET },
        ]
      : [];

  // Derive image-beat timing from the REAL section boundaries so a
