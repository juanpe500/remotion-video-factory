...
// Terminal / review-gate aesthetic — fits a video about turning "AI wrote
// your code" into "AI reviews your code, and you stay the boss". Monospace,
// scanlines, live typing, script-driven step code, progress bar, outro CTA.
...
// How long the outro CTA gets after narration ends.
...
// Subtle background texture — soft diagonal wash in the terminal palette so
// text isn't on flat black. CSS gradient only: this composition stays
// WebGL2-free (the render runner is software/swangle, no GPU).
...
// Slow, continuous zoom-in over the whole video. Pinned UI (progress bar,
// watermark) lives outside this wrapper so it doesn't drift with it.
...
// "Three," (words) or whisper writes "1." / "2." / "3." Whisper may split the
// comma into its own token ("one ,"), so allow optional whitespace.
const STEP_REGEX = [/\b(one|1)\s*[.,:]/i, /\b(two|2)\s*[.,:]/i, /\b(three|3)\s*[.,:]/i];

// The steps section builds a "review function" in a code editor that types
// itself out line-by-line, one call per real step. Invented pseudo-code that
// says exactly what the narration says.
const SYN = {
...
// One call per step, appended as each step's narration begins. atFrame is
// filled in from the real step start frames in the component.
const SIG: Tok[] = [
  { t: "def ", c: SYN.kw },
  { t: "ship_with_a_brain", c: SYN.fn },
  { t: "():", c: SYN.pun },
];
const CALL_REVIEW: Tok[] = [
  { t: "    ", c: SYN.pun },
  { t: "add_ai_reviewer", c: SYN.call },
  { t: "(", c: SYN.pun },
  { t: 'pr="every"', c: SYN.arg },
  { t: ")", c: SYN.pun },
];
const CALL_PROMPT: Tok[] = [
  { t: "    ", c: SYN.pun },
  { t: "review_architecture", c: SYN.call },
  { t: "(", c: SYN.pun },
  { t: "ignore_style", c: SYN.arg },
  { t: "=", c: SYN.pun },
  { t: "True", c: SYN.kw },
  { t: ")", c: SYN.pun },
];
const CALL_GATE: Tok[] = [
  { t: "    ", c: SYN.pun },
  { t: "gate_merge", c: SYN.call },
  { t: "(", c: SYN.pun },
  { t: 'critical="block"', c: SYN.arg },
  { t: ", ", c: SYN.pun },
  { t: 'warning="info"', c: SYN.arg },
  { t: ")", c: SYN.pun },
];
const RET: Tok[] = [
  { t: "    ", c: SYN.pun },
  { t: "return ", c: SYN.kw },
  { t: '"two_safety_nets"', c: SYN.str },
];
    ...
    <AbsoluteFill style={{ alignItems: "center", paddingTop: 205 }}>
      ...
      >
        ...
        >
          ...
          <span style={{ color: SYN.cmt, fontSize: 22, marginLeft: 12 }}>review.py</span>
        </div>
...
// Script-driven emphasis — hand-drawn circle / cross-off marks on specific
// words, once they finish typing. Keyed by the word stripped of punctuation.
// Picked for having runway after them within their own page.
    ...
    <AbsoluteFill style={{ backgroundColor: BG, justifyContent: "center", alignItems: "center", opacity }}>
      ...
      <Img
        ...
        style={{
          ...
          background: "#fff",
          // Small blur radius on purpose: big CSS blur is very expensive under
          // software rendering (swangle). 14px still reads as a glow.
...
// Bottom-third scene cards. Only the visual metadata lives here — the
...
const HOOK_BEAT: ImageBeatMeta = {
  id: "hook",
  file: "ai-pr-reviewer/images/hook-merge.jpg",
  label: "THE SCENE: merged unread, 3 months later",
...
const STEP_BEATS: ImageBeatMeta[] = [
  {
    id: "step1",
    file: "ai-pr-reviewer/images/step1-ai-review.jpg",
    label: "STEP 1: AI reviewer on every PR",
    width: 1000,
    bottom: 120,
  },
  {
    id: "step2",
    file: "ai-pr-reviewer/images/step2-prompt.jpg",
    label: "STEP 2: prompt for architecture, not syntax",
    width: 1000,
    bottom: 120,
  },
  {
    id: "step3",
    file: "ai-pr-reviewer/images/step3-merge-gate.jpg",
    label: "STEP 3: merge gate + canary = two safety nets",
    width: 1000,
    bottom: 120,
  },
];
...
export const Terminal: React.FC<AudioSyncedProps> = ({ narrationEndFrame }) => {
  ...
  // Build the accumulating review-function code, one call appended per step.
  const codeLines: CodeLine[] =
    stepRanges.length === 3
      ? [
          { atFrame: stepRanges[0].start, toks: SIG },
          { atFrame: stepRanges[0].start + 20, toks: CALL_REVIEW },
          { atFrame: stepRanges[1].start, toks: CALL_PROMPT },
          { atFrame: stepRanges[2].start, toks: CALL_GATE },
          { atFrame: stepRanges[2].start + 55, toks: RET },
        ]
      : [];

  // Derive image-beat timing from the REAL section boundaries so a voice or
  // script change (which shifts every timestamp) can't desync them.
